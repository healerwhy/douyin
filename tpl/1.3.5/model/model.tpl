...
func (m *default{{.upperStartCamelObject}}Model) InsertOrUpdate(ctx context.Context, field string, setStatus string, userId, optId, opt int64) (sql.Result, error) {
    return m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
        query := fmt.Sprintf("INSERT INTO %s (%s) VALUES (%d,%d,?) ON DUPLICATE KEY UPDATE %s=?", m.table, field, userId, optId, setStatus)
