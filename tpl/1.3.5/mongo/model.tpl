...
import (
    ...
    "github.com/globalsign/mgo/bson"
     {{if .CacheConf}}cachec "github.com/zeromicro/go-zero/core/stores/cache"
	"github.com/zeromicro/go-zero/core/stores/mongoc"{{else}}"github.com/zeromicro/go-zero/core/stores/mongo"{{end}}
)

{{if .CacheConf}}var prefix{{.Type}}CacheKey = "cache:{{.Type}}:"{{end}}
...
type default{{.Type}}Model struct {
    {{if .CacheConf}}*mongoc.Model{{else}}*mongo.Model{{end}}
}

func New{{.Type}}Model(url, collection string{{if .CacheConf}}, c cachec.CacheConf{{end}}) {{.Type}}Model {
	return &default{{.Type}}Model{
		Model: {{if .CacheConf}}mongoc.MustNewModel(url, collection, c){{else}}mongo.MustNewModel(url, collection){{end}},
	}
}
...
func (m *default{{.Type}}Model) FindOne(ctx context.Context, id string) (*{{.Type}}, error) {
    ...
    defer m.PutSession(session)
    var data {{.Type}}
    {{if .CacheConf}}key := prefix{{.Type}}CacheKey + id
    ...
    case nil:
        return &data,nil
    case {{if .CacheConf}}mongoc.ErrNotFound{{else}}mongo.ErrNotFound{{end}}:
...
func (m *default{{.Type}}Model) Update(ctx context.Context, data *{{.Type}}) error {
    ...
    defer m.PutSession(session)
	{{if .CacheConf}}key := prefix{{.Type}}CacheKey + data.ID.Hex()
